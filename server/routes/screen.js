...
async function processScreeningsBackground({ batchId, mode, jobDescription, jobTitle, inserted, userId }) {
  ...
  try {
    ...
    `).run(`Screened ${inserted.length} resume(s) (${mode === 'local' ? 'local scan' : 'OpenClaw local'})`, userId);
  } catch (_) {}
