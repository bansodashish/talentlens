...
router.post('/login', async (req, res) => {
  ...
  const { password: _, apify_key_enc, claude_key_enc, ...safeUser } = user;
...
router.get('/me', authMiddleware, (req, res) => {
  ...
  const { password, apify_key_enc, claude_key_enc, ...safeUser } = row;
...
router.get('/me/keys', authMiddleware, (req, res) => {
  const row = db.prepare('SELECT apify_key_enc, claude_key_enc FROM users WHERE id = ?').get(req.user.id);
  if (!row) return res.status(404).json({ error: 'User not found.' });
  res.json({
...
router.put('/me', authMiddleware, async (req, res) => {
  const { name, company, market, apify_key, claude_key, current_password, new_password } = req.body;
  ...
  updated.has_apify_key  = !!(apify_key  || row.apify_key_enc);
  updated.has_claude_key = !!(claude_key || row.claude_key_enc);
